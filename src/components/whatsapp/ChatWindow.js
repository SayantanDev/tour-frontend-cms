...
import { sendMessage, getChatMessages } from '../../api/whatsappAPI';
import { addMessage, markChatAsRead } from '../../reduxcomponents/slices/whatsappSlice';
...
const ChatWindow = ({ socket }) => {
  ...
  const { selectedChat, currentAgent, typingIndicators, templates } = useSelector((state) => state.whatsapp);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessageText('');
      if (socket) {
        socket.emit('message:send', { chatId: selectedChat.id, message });
      }
    } catch (error) {
